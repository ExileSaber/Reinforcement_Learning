...
class Space_Invaders_CNN(nn.Module):
    ...
    def forward(self,x):
        ...
        x = x.view(-1,32*81)  # review as (1, 32*81)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x
...
# 第二部分：循环缓存区部分
# 一个用于存放近期环境转移的有限尺寸循环缓存区，它提供了.sample方法来选择训练所需的随机批次转移
class ReplayMemory(object):
    def __init__(self, capacity):
        self.capacity = capacity  # 缓冲区大小，arg.buffer size
        self.memory = []
        self.position = 0

    def store(self, *args):
        if len(self.memory) < self.capacity:  # 先扩充缓存区达到指定buffer size大小
            self.memory.append(None)
        # 1个表示单个环境状态转移的命名元组，它将（状态，动作）对映射到（下一个状态，奖励）上。
        self.memory[self.position] = Transition(*args)  # 赋值
        self.position = (self.position + 1) % self.capacity  # 每一次往后 +1

    def sample(self, batch_size):
        return random.sample(self.memory, batch_size)  # 从 self.memory 中随机挑选 batch_size 个命名元组
...
class Agent(object):
    def __init__(self, args, render=False):
        self.env = gym.make(args.env)  # 开启环境
        ...
        if args.exp_replay:
            ...
            self.memory = ReplayMemory(args.buffer_size)  # 从memory中随机生成
        else:
    ...
    def select_action(self, state, train):
        ...
        if self.eps_greedy:
            ...
            if random.random() > eps_threshold:
                the_value = self.model(Variable(state, volatile=True).type(FloatTensor)).data
                the_value_2 = the_value.max(1)[1].view(1, 1)
                print("===== ", the_value)
                print("===== ", type(the_value))
                print("===== ", the_value_2)
                print("===== ", type(the_value_2))
...
################################################################################################################################################
    ...
    def optimize_model(self):
        ...
        for reward in batch.reward:
            expected_Q = np.append(expected_Q, reward.cpu())

        if use_cuda:
            np.concatenate((expected_Q[non_terminal_mask], (self.gamma * max_next_Q).cpu().data), axis=0)
        ...
        if self.loss_fn == 'l1':
            loss = F.smooth_l1_loss(current_Q.to(torch.float32), expected_Q.to(torch.float32))
        else:
            loss = F.mse_loss(current_Q.to(torch.float32), expected_Q.to(torch.float32))
...
def parse_arguments():
    ...
    parser.add_argument('--buffer_size', type=int, default=1000, help='Replay memory buffer size')
    ...
    parser.add_argument('--loss_fn', type=str, default='l1', help='loss function one of (l1,l2) | Default: l1')
