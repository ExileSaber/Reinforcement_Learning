...
prepro = lambda img: np.array(Image.fromarray(img[35:195].mean(2)).resize((80, 80))).reshape(1, 80, 80) / 255.
...
class NNPolicy(nn.Module):
    def __init__(self, num_actions):
        ...
        # ???
        self.gru = nn.GRUCell(32 * 5 * 5, 256)  # 规定输入维度 32 * 5 * 5 隐藏维度为 256
    ...
    def forward(self, inputs, train=True, hard=False):
        ...
        hx = self.gru(x.view(-1, 32 * 5 * 5), (hx)) # [1,256]  # 保留隐藏层的结果
...
class SharedAdam(torch.optim.Adam):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0):
        super(SharedAdam, self).__init__(params, lr, betas, eps, weight_decay)  # 父类的初始化函数
        for group in self.param_groups:
            for p in group['params']:  # 可更新参数分为不同的几组，每组使用不同的策略
                # 初始化state
                state = self.state[p]
...
def loss_func(args, values, logps, actions, rewards):
    np_values = values.view(-1).data.numpy()  # 获取values值
